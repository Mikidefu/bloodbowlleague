...
export default function SchedulePage() {
  ...
  // Le giornate dei playoff vanno nel pannello rosso "Final Four"
  ...
  const matchGrid = currentRound !== null && (
      <div className={styles.fixtureGrid}>
        {groupedMatches[currentRound]?.map(match => (
            <article key={match.id} className={`card ${styles.fixture}`}>
              <div className={styles.fixtureMeta}>
                <span className={match.is_played ? 'tag tag-navy' : 'tag'}>
                  {match.is_played ? 'COMPLETED' : 'UPCOMING'}
                </span>
                <span className={styles.matchType}>{displayMatchType(match.match_type)}</span>
              </div>
              ...
              <div className={styles.fixtureActions}>
                ...
                {canEdit && (
                    <button
                        ...
                        className={`btn btn-slate ${styles.iconBtn}`}
  ...
  return (
      <div className={styles.page}>
        ...
        <div className={`panel-slate ${styles.filterBar}`}>
        ...
        {rounds.length === 0 || currentRound === null || !groupedMatches[currentRound] ? (
            ...
            <section>
              <div className={`panel-slate ${styles.roundNav}`}>
                <button
                    type="button"
              ...
              {isPlayoffRound ? (
                  <div className={`panel-blood ${styles.playoffPanel}`}>
                    <h3 className={`title-spike ${styles.playoffTitle}`}>
                      <Trophy size={32} aria-hidden="true" /> Final Four
                    </h3>
                    {matchGrid}
                  </div>
              ) : matchGrid}
